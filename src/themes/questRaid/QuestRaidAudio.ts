...
import type { TavernCue } from './tavern/questRaidTavernDirector'
export class QuestRaidAudio {
  ...
  cue(type: QuestBattleEvent['type'] | 'cursor' | 'confirm' | 'damage' | 'victory' | 'near_death' | 'battle_start' | TavernCue) {
    const pitches: Record<string, number[]> = {
      ...
      talk: [392], pop: [880, 1180], point: [523, 784], complete: [392, 523, 659], finale: [392, 494, 587, 784], none: [],
    }
    const context = this.context, master = this.master
    if (!context || !master || !this.enabled) return
    const notes = pitches[type] ?? [440]
