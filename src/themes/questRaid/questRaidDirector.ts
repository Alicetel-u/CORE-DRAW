...
export type QuestFrame = {
  fighters: Fighter[]
  event: QuestBattleEvent
  elapsed: number
  eventIndex: number
  page: number
  bossHp: number
...
function isGroupingAssignment(event: QuestBattleEvent) {
  return Boolean(event.rankById && /^【店員】\n「パーティ\d+へ、/.test(event.message))
}

export function playQuestRaidBattle(
  script: QuestBattleScript,
  audio: QuestRaidAudio | null,
  onFrame: (frame: QuestFrame) => void,
  onReveal: () => void,
  onComplete: () => void,
) {
  ...
  let page = 0
  let lastPageAt = 0
  let fighters = script.fighters.map(f => ({ ...f }))
  ...
  function tick() {
    ...
    while (index + 1 < script.events.length && script.events[index + 1].at <= elapsed) {
      ...
      const groupingAssignment = isGroupingAssignment(event)
      criticalIds = []

      fighters = fighters.map(f => {
        const hp = event.hp?.[f.id] ?? f.hp
        const mp = event.mp?.[f.id] ?? f.mp
        ...
        return {
          ...
          hp,
          mp,
          party: event.partyById?.[f.id] ?? (groupingAssignment ? event.rankById?.[f.id] : undefined) ?? f.party,
          rank: groupingAssignment ? f.rank : event.rankById?.[f.id] ?? f.rank,
        }
      })

      if (event.order) {
        const byId = new Map(fighters.map(f => [f.id, f]))
        fighters = event.order.map(id => byId.get(id)!).filter(Boolean)
      }

      bossHp = event.bossHp ?? bossHp

      // Suppress a burst of stale audio after returning from a background tab.
      if (elapsed - event.at < 200) {
        audio?.cue(event.type)
        if (criticalIds.length) audio?.cue('near_death')
      }

      if (event.type === 'result' && !revealed) {
        revealed = true
        onReveal()
      }
    }
    ...
    if (elapsed - lastPaint >= 32 || elapsed >= script.duration) {
      onFrame({
        fighters,
        event,
        elapsed: Math.min(elapsed, script.duration),
        eventIndex: index,
        page,
        bossHp,
        criticalIds,
