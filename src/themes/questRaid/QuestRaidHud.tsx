...
export function QuestRaidHud({ event, age, reduced }: { event: QuestBattleEvent; age: number; reduced: boolean }) {
  const active = event.type === 'player_spell' || event.type === 'player_heal' ? 1 : event.type.startsWith('boss') || event.type === 'knockout' ? 3 : event.type === 'player_attack' || event.type === 'final_strike' ? 0 : -1
