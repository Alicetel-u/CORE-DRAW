...
export function directDraw(s: CinematicState, reduced: boolean, audio: DrawAudio | null,
  ...
  tl.to(s, { time: CUES.end, progress: 100, duration: CUES.end, ease: 'none' }, 0)
    ...
    // Beat 5: break the silence in the same spatial setup, then let the winner cross the lens.
    ...
    .set(s, { winner: 1, ws: .34, wz: .45, wry: -1.25, wrz: -.32 }, CUES.eject)
    .to(s, { wx: 2.45, wy: .72, wz: 9.25, ws: .9, wry: 2.15, wrz: .42, duration: .57, ease: 'power2.in' }, CUES.eject)

    // Beat 6: follow rather than cut. The card arcs back into a hero position and settles with weight.
